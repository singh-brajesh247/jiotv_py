...
const playWarmups = new Map();

function warmupChannelPlayback(card) {
  if (!card || card.dataset.requiresLogin === "true") {
    return;
  }
  ...
  const warmupUrl = new URL(`/warmup/${encodeURIComponent(channelId)}`, window.location.origin);
  const quality = target.searchParams.get("q") || (qualityElement ? qualityElement.value : "");
  const playerMode = target.searchParams.get("pm");

  if (quality && quality !== "auto") {
    warmupUrl.searchParams.set("q", quality);
  }
  if (playerMode) {
    warmupUrl.searchParams.set("pm", playerMode);
  }

  const key = warmupUrl.toString();
  const now = Date.now();
  if (now - (playWarmups.get(key) || 0) < 15000) {
    return;
  }
  playWarmups.set(key, now);
...
function initializePlaybackWarmup() {
  document.querySelectorAll('a.card[data-channel-id]').forEach((card) => {
    card.addEventListener("pointerenter", () => warmupChannelPlayback(card), { passive: true });
    card.addEventListener("pointerdown", () => warmupChannelPlayback(card), { passive: true });
    card.addEventListener("focus", () => warmupChannelPlayback(card));
  });
}
